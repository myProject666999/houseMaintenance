...
const Owners = () => {
  ...
  const fetchOwners = async () => {
    setLoading(true);
    try {
      const res = await adminAPI.getOwnerUsers();
      setOwners(res.data?.users || []);
    } catch (error) {
      message.error('获取业主列表失败');
    } finally {
      setLoading(false);
    }
  };

  const showDetail = (owner) => {
  ...
  const handleSubmit = async (values) => {
    try {
      if (editingItem) {
        ...
        await adminAPI.createUser({ ...values, role: 'owner' });
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        <h2 className="page-title" style={{ margin: 0 }}>业主管理</h2>
